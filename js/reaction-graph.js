...
MetabolicGraph.prototype.setReactions_hidemetabs = function(list) {
		...
		for (var x in reactions[i].outputs) {
			...
			//} else {
				for (var j=0; j<reactions[i].metabolites[x].consumers.length; j++) {
					...
					if (!edgeSig) {
						if (this.options.subsystemCluster && con.subsystem == reactions[i].subsystem) {
							//if (!nodes[con.id]) continue;
						} else {
							continue;
						}
					}

					if (con.subsystem != reactions[i].subsystem && !nodes[con.id]) continue;
					...
					links.push({
						...
						sig: edgeSig,
						subsys: con.subsystem == reactions[i].subsystem
					});

					if (con.subsystem == reactions[i].subsystem) {
						subsyscount[con.subsystem]++;
					}
				}
			//}
		}
	...
	this.graphData({nodes: nodes, links: links}, function(link) {
		...
		return (link.subsys) ? ((link.invisible) ? Math.floor(me.options.subsysScale*Math.sqrt(subsyscount[link.name])) : 15*me.options.subsysScale) : Math.floor(40 + count*20);
	}, -300, subsys_colours);
}
...
MetabolicGraph.prototype.graphData = function(data, dist, charge, cols) {
	...
	var force = d3.layout.force()
		...
		.charge(charge) // -300
		.linkStrength(link => (link.subsys) ? ((link.invisible) ? 2 : 3) : (link.blocked) ? 0 : 0.05)
		.on("tick", tick)
		.start();
	this.force = force;
